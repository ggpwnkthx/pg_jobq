...
BEGIN v_seq_name := pg_get_serial_sequence('jobq.jobs', 'job_id');
IF v_seq_name IS NOT NULL THEN -- Align sequence owner with table owner.
EXECUTE format(
  'ALTER SEQUENCE %s OWNER TO jobq_worker',
  v_seq_name
);
...
REVOKE ALL ON TABLE jobq.jobs
FROM PUBLIC;
GRANT SELECT,
  INSERT,
  UPDATE,
  DELETE ON jobq.jobs TO jobq_worker;
...
REVOKE ALL ON FUNCTION jobq.cancel(BIGINT)
FROM PUBLIC;
GRANT EXECUTE ON FUNCTION jobq.cancel(BIGINT) TO jobq_client;
-- Internal read-only executor helper: worker-only.
REVOKE ALL ON FUNCTION jobq.exec_readonly_to_blob(TEXT, TEXT, TEXT, TEXT)
FROM PUBLIC;
...
REVOKE ALL ON FUNCTION jobq.claim_next_job()
FROM PUBLIC;
GRANT EXECUTE ON FUNCTION jobq.claim_next_job() TO jobq_worker;
REVOKE ALL ON PROCEDURE jobq.runner(BIGINT, INTEGER)
FROM PUBLIC;
...
ELSE RAISE NOTICE 'azure_pg_admin role not found; skipping azure_pg_admin-specific GRANTs';
END IF;
END;
$$ LANGUAGE plpgsql;
...
REVOKE ALL ON TABLE jobq.v_queue_overview
FROM PUBLIC;
REVOKE ALL ON TABLE jobq.v_running_jobs
FROM PUBLIC;
REVOKE ALL ON TABLE jobq.v_recent_jobs
FROM PUBLIC;
REVOKE ALL ON TABLE jobq.v_stalled_jobs
FROM PUBLIC;
GRANT SELECT ON jobq.v_queue_overview,
  jobq.v_running_jobs,
  jobq.v_recent_jobs,
  jobq.v_stalled_jobs TO jobq_reporting;
-- Optional: give ops read-only access to the views as well.
GRANT SELECT ON jobq.v_queue_overview,
  jobq.v_running_jobs,
  jobq.v_recent_jobs,
  jobq.v_stalled_jobs TO jobq_ops;
------------------------------
...
DO $jobq_cron$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_extension
  WHERE extname = 'pg_cron'
) THEN RAISE NOTICE 'pg_cron is not installed in database %, skipping jobq.install_cron_jobs() helper creation',
current_database();
RETURN;
END IF;
...
CREATE OR REPLACE FUNCTION jobq.install_cron_jobs(
  ...
  ) RETURNS void LANGUAGE plpgsql -- SECURITY INVOKER is the default; do NOT use SECURITY DEFINER here.
SET search_path = pg_catalog,
  jobq AS $body$ BEGIN IF p_target_database_name IS NULL
  OR btrim(p_target_database_name) = '' THEN RAISE EXCEPTION 'p_target_database_name cannot be null or empty';
END IF;
--------------------------------------------------------------------
...
-- Separate conditional GRANT for azure_pg_admin on install_cron_jobs
DO $$ BEGIN IF EXISTS (
  SELECT 1
  FROM pg_roles
  WHERE rolname = 'azure_pg_admin'
...
AND EXISTS (
  ...
  WHERE pronamespace = 'jobq'::regnamespace
    AND proname = 'install_cron_jobs'
) THEN
GRANT EXECUTE ON FUNCTION jobq.install_cron_jobs(TEXT) TO azure_pg_admin;
ELSE RAISE NOTICE 'azure_pg_admin role or jobq.install_cron_jobs(TEXT) missing; skipping GRANT';
END IF;
END;
$$ LANGUAGE plpgsql;
-- Example:

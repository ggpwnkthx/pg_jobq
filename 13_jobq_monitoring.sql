...
BEGIN
  IF NOT EXISTS (
    ...
    CREATE TYPE jobq.queue_metrics AS (
      pending               BIGINT,
      running               BIGINT,
      succeeded             BIGINT,
      failed                BIGINT,
...
SELECT
  ...
  MAX(now() - scheduled_at)
    FILTER (WHERE status = 'pending')          AS oldest_pending_wait,

  AVG(now() - scheduled_at)
    FILTER (WHERE status = 'pending')          AS avg_pending_wait
FROM jobq.jobs;

CREATE OR REPLACE FUNCTION jobq.get_queue_metrics()
RETURNS jobq.queue_metrics
LANGUAGE sql
AS $$
  SELECT
    pending,
...
ORDER BY j.started_at NULLS LAST, j.job_id;

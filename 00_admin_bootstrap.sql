...
DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_roles
  WHERE rolname = 'jobq_worker'
) THEN -- Service/owner role for jobq objects and SECURITY DEFINER functions.
...
IF NOT EXISTS (
  SELECT 1
  FROM pg_roles
  WHERE rolname = 'jobq_reader'
) THEN -- Marker role for read-only execution against data exposed to the queue.
...
IF NOT EXISTS (
  SELECT 1
  FROM pg_roles
  WHERE rolname = 'jobq_client'
) THEN -- Intended for applications/services that enqueue and cancel jobs.
CREATE ROLE jobq_client NOINHERIT;
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_roles
  WHERE rolname = 'jobq_reporting'
) THEN -- Intended for BI/reporting; will receive SELECT on views later.
CREATE ROLE jobq_reporting NOINHERIT;
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_roles
  WHERE rolname = 'jobq_ops'
) THEN -- Intended for operational controls (kill, requeue, purge).
...
) THEN EXECUTE 'GRANT pg_signal_backend TO jobq_worker';
END IF;
EXCEPTION
WHEN insufficient_privilege THEN RAISE NOTICE 'Could not GRANT pg_signal_backend TO jobq_worker (insufficient_privilege). If your service supports it, grant this manually so jobq.kill() can terminate backends.';
WHEN undefined_object THEN -- Role not present on this server; nothing to do.
NULL;
END;
END;
$$ LANGUAGE plpgsql;

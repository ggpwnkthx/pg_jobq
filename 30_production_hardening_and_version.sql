/*
 =====================================================================
 30_production_hardening_and_version.sql – Indexes, constraints, version stamp
...
CREATE INDEX IF NOT EXISTS idx_jobq_jobs_finished_at ON jobq.jobs (finished_at)
...
CREATE INDEX IF NOT EXISTS idx_jobq_jobs_pending_schedule ON jobq.jobs (priority DESC, scheduled_at, job_id)
WHERE status = 'pending';
------------------------------
...
ADD CONSTRAINT jobq_jobs_max_runtime_window_chk CHECK (
...
VALUES ('1.0.0') ON CONFLICT (version) DO NOTHING;
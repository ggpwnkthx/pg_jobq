...
BEGIN
  ...
  IF v_trim_sql IS NULL OR length(v_trim_sql) = 0 THEN
    RAISE EXCEPTION 'p_query_sql cannot be empty';
  END IF;

  IF length(v_trim_sql) > 100000 THEN
    RAISE EXCEPTION 'query_sql too long (limit: 100000 characters)';
  END IF;

  IF p_storage_account IS NULL OR btrim(p_storage_account) = '' THEN
    RAISE EXCEPTION 'p_storage_account cannot be empty';
  END IF;

  IF p_storage_container IS NULL OR btrim(p_storage_container) = '' THEN
    RAISE EXCEPTION 'p_storage_container cannot be empty';
  END IF;

  IF p_priority < -1000 OR p_priority > 1000 THEN
    RAISE EXCEPTION 'p_priority must be between -1000 and 1000 (got %)', p_priority;
  END IF;
  ...
  IF v_runtime <= interval '0 seconds' THEN
    RAISE EXCEPTION 'p_max_runtime must be greater than 0 seconds (got %)', p_max_runtime;
  END IF;

  IF v_runtime > interval '24 hours' THEN
    RAISE EXCEPTION 'p_max_runtime must not exceed 24 hours (got %)', p_max_runtime;
  END IF;
  ...
  WHILE v_i <= v_len LOOP
    ...
    IF NOT v_in_string THEN
      ...
      v_i := v_i + 1;
    ELSE
      -- Inside string literal: preserve delimiters, blank out contents.
      IF v_ch = '''' THEN
        IF v_i < v_len AND substr(v_trim_sql, v_i + 1, 1) = '''' THEN
          -- Escaped quote: skip second quote, keep a placeholder.
  ...
  --------------------------------------------------------------------
  IF v_sql_scan !~* E'^(select|with)\\y' THEN
    RAISE EXCEPTION 'query_sql must start with SELECT or WITH';
  END IF;

  -- Disallow semicolons to prevent multi-statement payloads.
  IF v_sql_scan ~ ';' THEN
    RAISE EXCEPTION 'query_sql must not contain semicolons';
  END IF;

  -- Disallow SQL comments to reduce surface for trickery.
  IF v_sql_scan ~ '--' OR v_sql_scan ~ '/\\*' THEN
    RAISE EXCEPTION 'query_sql must not contain SQL comments';
  END IF;

  -- Disallow any use of INTO (e.g. SELECT ... INTO).
  IF v_sql_scan ~* E'\\yinto\\y' THEN
    RAISE EXCEPTION 'query_sql must not contain INTO';
  END IF;

  -- Additional hardening: disallow obvious mutating/DDL keywords
  -- (outside literals).
  IF v_sql_scan ~* E'\\b(insert|update|delete|merge|truncate|create|alter|drop|grant|revoke|copy|vacuum|analyze|cluster|refresh|reindex|call|do|lock)\\b' THEN
    RAISE EXCEPTION 'query_sql must be a read-only SELECT/WITH (no DML/DDL keywords allowed)';
  END IF;
...
CREATE OR REPLACE FUNCTION jobq.cancel(p_job_id BIGINT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, jobq
...
AS $$
BEGIN
  -- Execute the supplied SELECT/WITH and stream results to Azure Blob as Parquet.

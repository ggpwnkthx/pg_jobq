...
CREATE OR REPLACE FUNCTION jobq.kill(p_job_id BIGINT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- Look up the job and the currently running backend (if any),
  ...
  IF v_pid IS NOT NULL
     ...
     AND (
       v_backend_query ILIKE '%jobq.run%' OR
       v_backend_query ILIKE '%azure_storage.blob_put%'
     )
  THEN
    -- Check whether jobq_worker is allowed to signal backends.
    SELECT pg_has_role('jobq_worker', 'pg_signal_backend', 'member')
    INTO v_can_signal;

    IF v_can_signal THEN
      BEGIN
        PERFORM pg_terminate_backend(v_pid);
        v_killed := true;
      EXCEPTION
        WHEN insufficient_privilege THEN
          -- Platform may not allow this despite membership attempts;
...
CREATE OR REPLACE FUNCTION jobq.requeue_orphaned_running_jobs(
...
BEGIN
  FOR v_job IN
    ...
    WHERE j.status = 'running'
      AND (j.backend_pid IS NULL OR sa.pid IS NULL)
    ORDER BY j.job_id
    LIMIT p_limit

...
DO $$ BEGIN IF NOT EXISTS (
  ...
  WHERE t.typname = 'job_status'
    AND n.nspname = 'jobq'
) THEN CREATE TYPE jobq.job_status AS ENUM (
  'pending',
  'running',
  'succeeded',
  'failed',
...
CREATE TABLE IF NOT EXISTS jobq.jobs (
  ...
  -- Basic sanity guards.
  CONSTRAINT jobq_jobs_max_attempts_chk CHECK (max_attempts >= 1),
  CONSTRAINT jobq_jobs_max_runtime_chk CHECK (max_runtime > interval '0 seconds'),
  CONSTRAINT jobq_jobs_priority_chk CHECK (
...
ALTER TABLE jobq.jobs
ADD CONSTRAINT jobq_jobs_max_attempts_chk CHECK (max_attempts >= 1);
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_constraint
...
ALTER TABLE jobq.jobs
ADD CONSTRAINT jobq_jobs_max_runtime_chk CHECK (max_runtime > interval '0 seconds');
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_constraint
...
ALTER TABLE jobq.jobs
ADD CONSTRAINT jobq_jobs_priority_chk CHECK (
    priority BETWEEN -1000 AND 1000
  );
END IF;
END;
...
CREATE INDEX IF NOT EXISTS idx_jobq_jobs_status_scheduled ON jobq.jobs (status, scheduled_at);
CREATE INDEX IF NOT EXISTS idx_jobq_jobs_status_started ON jobq.jobs (status, started_at);
-- Correlation-centric index for typical reporting/debug flows.
CREATE INDEX IF NOT EXISTS idx_jobq_jobs_corrid_jobid ON jobq.jobs (correlation_id, job_id DESC);
------------------------------
-- 3. updated_at trigger – maintain last modification timestamp
------------------------------
CREATE OR REPLACE FUNCTION jobq.set_timestamp() RETURNS TRIGGER LANGUAGE plpgsql
SET search_path = pg_catalog,
  jobq AS $$ BEGIN NEW.updated_at := now();
RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_jobq_jobs_set_timestamp ON jobq.jobs;
CREATE TRIGGER trg_jobq_jobs_set_timestamp BEFORE
UPDATE ON jobq.jobs FOR EACH ROW EXECUTE FUNCTION jobq.set_timestamp();
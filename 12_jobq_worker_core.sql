...
CREATE OR REPLACE FUNCTION jobq.claim_next_job(
  OUT o_job_id  BIGINT,
...
BEGIN
  ...
  BEGIN
    ...
    IF v_cfg IS NULL OR NULLIF(trim(v_cfg), '') IS NULL THEN
      v_global_parallel_cap := 4;      -- default if not configured
    ELSE
      BEGIN
        v_global_parallel_cap := LEAST(10000, GREATEST(1, v_cfg::INTEGER));
      EXCEPTION
        WHEN OTHERS THEN
          -- Defensive: fall back if somebody sets a garbage value.
    ...
    IF v_cfg IS NULL OR NULLIF(trim(v_cfg), '') IS NULL THEN
      v_min_free_conns := 5;           -- default safety buffer
    ELSE
      BEGIN
        v_min_free_conns := LEAST(1000, GREATEST(0, v_cfg::INTEGER));
      EXCEPTION
        WHEN OTHERS THEN
          v_min_free_conns := 5;
      END;
    ...
    IF v_free_connections <= v_min_free_conns THEN
      RETURN;
    END IF;
    ...
    --------------------------------------------------------------------
    v_slot_id := 1;
    WHILE v_slot_id <= v_global_parallel_cap AND NOT v_got_slot LOOP
      v_got_slot := pg_try_advisory_lock(42, v_slot_id);
      IF NOT v_got_slot THEN
        v_slot_id := v_slot_id + 1;
      END IF;
    END LOOP;
    ...
    IF NOT FOUND THEN
      -- Nothing to do; release slot and exit.
  ...
  EXCEPTION
    WHEN OTHERS THEN
      -- Defensive: if we grabbed a slot and then errored, release it so
      -- we don't leak global capacity.
      IF v_got_slot AND v_slot_id IS NOT NULL THEN
        PERFORM pg_advisory_unlock(42, v_slot_id);
      END IF;
      RAISE;
  END;
...
CREATE OR REPLACE PROCEDURE jobq.runner(
  IN p_job_id  BIGINT,
  IN p_slot_id INTEGER
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, jobq
...
BEGIN
  BEGIN
    ...
    IF NOT FOUND THEN
      -- Job disappeared; just release slot and bail.
    ...
    IF v_job.status <> 'running' THEN
      -- Somebody else changed it; release slot and bail.
      PERFORM pg_advisory_unlock(42, p_slot_id);
      v_slot_released := true;
      RETURN;
    END IF;
    ...
    --------------------------------------------------------------------
    BEGIN
      -- Per-job timeout: default 30 minutes, clamp to [1s, 24h].
      v_timeout := COALESCE(v_job.max_runtime, interval '30 minutes');

      IF v_timeout <= interval '0' THEN
        v_timeout := interval '1 second';
      ELSIF v_timeout > interval '24 hours' THEN
        v_timeout := interval '24 hours';
      END IF;

      v_timeout_ms := CEIL(EXTRACT(EPOCH FROM v_timeout) * 1000)::INTEGER;
      ...
      IF v_prev_timeout IS NOT NULL THEN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        IF v_attempt >= v_job.max_attempts THEN
          -- Final failure after max_attempts reached.
          UPDATE jobq.jobs
          SET status      = 'failed',
  ...
  EXCEPTION
    WHEN OTHERS THEN
      -- Absolute last line of defense: if something unexpected escaped
      -- the normal paths, make sure we don't leak the slot.
      IF NOT v_slot_released THEN
        PERFORM pg_advisory_unlock(42, p_slot_id);
      END IF;
      RAISE;
  END;
...
CREATE OR REPLACE PROCEDURE jobq.run_next_job()
LANGUAGE plpgsql
-- SECURITY INVOKER is the default; do NOT add SECURITY DEFINER or SET here.
AS $$
DECLARE
  v_job_id  BIGINT;
  v_slot_id INTEGER;
BEGIN
  ...
  --------------------------------------------------------------------
  SELECT o_job_id, o_slot_id
  INTO v_job_id, v_slot_id
  FROM jobq.claim_next_job();

  IF v_job_id IS NULL THEN
    -- Nothing to do; commit any incidental work and exit.
    COMMIT;
    RETURN;
  END IF;
